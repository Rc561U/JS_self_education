...
@dp.callback_query_handler(lambda c: c.data in ['das','der','die'], state=GameStates.random_ten)
async def button_click_call_back(callback_query: types.CallbackQuery, state:FSMContext):
    ...
    async with state.proxy() as data:
        if answer == data.get('answer'):
            ...
            data['word'] = res.get('word')
            if data['step'] > 10:
                await bot.send_message(callback_query.from_user.id, 'Das spiel ist forbei!',)
                await GameStates.start.set()
            else:
                await bot.send_message(callback_query.from_user.id, 'Ja!\n' + f"{data['step']} of 10. Das wort ist {data['word']}", reply_markup=inline_kb)
        else:
            await bot.send_message(callback_query.from_user.id, 'Nein\n', reply_markup=inline_kb)